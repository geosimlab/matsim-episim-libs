 ...
 */
public class ActivityHandler implements ActivityEndEventHandler, PersonEntersVehicleEventHandler, PersonLeavesVehicleEventHandler, ActivityStartEventHandler {
	 ...
	 * Constructor.
	 */
	public ActivityHandler(@Nullable Population population, @Nullable Set<String> personIds, @Nullable Map<Id<ActivityFacility>, Id<ActivityFacility>> facilityReplacements) {
	...
	public void handleEvent(ActivityEndEvent activityEndEvent) {
		...
		String subpop = population.getPersons().get(activityEndEvent.getPersonId()).getAttributes().getAttribute("subpopulation").toString();
		String newActType = activityEndEvent.getActType() + "_" +subpop; 
		ActivityEndEvent activityEndEvent1 = new ActivityEndEvent(activityEndEvent.getTime(),
				activityEndEvent.getPersonId(),
				activityEndEvent.getLinkId(),
				activityEndEvent.getFacilityId(),
				newActType);
		facilities.add(activityEndEvent.getFacilityId());
//		events.remove(activityEndEvent);
		events.add(activityEndEvent1);
	}
	...
	public void handleEvent(ActivityStartEvent activityStartEvent) {
		...
		String subpop = population.getPersons().get(activityStartEvent.getPersonId()).getAttributes().getAttribute("subpopulation").toString();
		String newActType = activityStartEvent.getActType() + "_" +subpop; 
		ActivityEndEvent activityStartEvent1 = new ActivityEndEvent(activityStartEvent.getTime(),
				activityStartEvent.getPersonId(),
				activityStartEvent.getLinkId(),
				activityStartEvent.getFacilityId(),
				newActType);
		facilities.add(activityStartEvent.getFacilityId());
		events.remove(activityStartEvent);
		events.add(activityStartEvent1);
	}
