...
public class SelectedPlans2ESRIShape {
	...
	private void writeActs() throws IOException {
		...
		for (Plan plan : this.outputSamplePlans) {
			...
			for (PlanElement pe : plan.getPlanElements()) {
				if (pe instanceof Activity && first) {
					Activity act = (Activity) pe;
					fts.add(getActFeature(id, act,person));
					first = false;
				}
			}
		}
	...
	private SimpleFeature getActFeature(final String id, final Activity act, Person person) {
...
//		actBuilder.add("END_TIME", Double.class);
	...
	public static void main(final String [] args) {
		...
		final String populationFilename = "C:/GeoSimLab/episim_jlm/Input_data/matsim_files/population1.0.xml.gz";
		final String networkFilename = "C:/GeoSimLab/episim_jlm/Input_data/matsim_files/11.output_network.xml.gz";
		...
		final String outputDir = "C:/GeoSimLab/episim_jlm/temp";
		...
		CoordinateReferenceSystem crs = MGC.getCRS("EPSG:2039");
