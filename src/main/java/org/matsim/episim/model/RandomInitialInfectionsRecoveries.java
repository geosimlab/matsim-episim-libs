 ...
 */
public class RandomInitialInfectionsRecoveries implements InitialInfectionHandler {

	private static final Logger log = LogManager.getLogger(RandomInitialInfectionsRecoveries.class);
	...
	@Inject
	public RandomInitialInfectionsRecoveries(Config config, SplittableRandom rnd) {
	...
	public int handleInfections(Map<Id<Person>, EpisimPerson> persons, int iteration) {
		...
		int infected = 0;
//		create new key in population "recovered"
//		add another
		if (iteration == 1) {
			List<EpisimPerson> recovered = persons.values().stream()
					.filter(p -> district == null || district.equals(p.getAttributes().getAttribute("recovered")))
					.collect(Collectors.toList());
			for (int i = 0; i < recovered.size(); i++) {
				EpisimPerson recoveredPerson =recovered.get(i);
				recoveredPerson.setDiseaseStatus(now, EpisimPerson.DiseaseStatus.recovered);
			}
		} 
