 ...
 * </pre>
 */
public final class GroupInfectionModel implements InfectionModel {
	...
	@Inject
	public GroupInfectionModel(FaceMaskModel faceMaskModel, Config config) {
	...
	public double calcInfectionProbability(EpisimPerson target, EpisimPerson infector, Map<String, Restriction> restrictions,
		...
		boolean is1secular= target.getAttributes().getAttribute("subpopulation").toString().equals("internal_Secular") ;
		boolean is2secular = infector.getAttributes().getAttribute("subpopulation").toString().equals("internal_Secular");
		boolean is1orthodox = target.getAttributes().getAttribute("subpopulation").toString().equals("internal_Ultra-Orthodox") ;
		boolean is2orthodox = infector.getAttributes().getAttribute("subpopulation").toString().equals("internal_Ultra-Orthodox");
		double groupFactor = 1;
		if(is1orthodox &&is2orthodox ) {
			groupFactor = JlmEpisimEverythingGoes.ultraOrthodoxInfectionRate;
		}else if(is1secular && is2secular) {
			groupFactor = JlmEpisimEverythingGoes.secularInfectionRate;
		}
